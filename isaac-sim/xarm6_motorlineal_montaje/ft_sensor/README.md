El sensor de fuerza está integrado en el .usd (``xArm6_sensorFuerza_motorLineal.usd``) en el Action Graph ``ft_sensor``.
...
Consecuentemente, se ha integrado un script de Python en el OmniGraph, ``ft_sensor.py``, que obtiene los valores de fuerza, y los saca como output para mandar el bloque de ROS2 Publisher. Esta manera tiene un error, y es que entre simulaciones hay que pulsar "Reload Script" en el nodo de Script del OmniGraph. Se ha preguntado en GitHub, https://github.com/isaac-sim/IsaacSim/discussions/513, pero de momento no se ha encontrado solución.
