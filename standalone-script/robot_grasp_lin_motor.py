...
class RobotGrasp(object):    
    ...
    def update_pos_loop(self):
        ...
        ## AQUI SE PUEDE AGREGAR LA POSICION DEL MOTOR LINEAL PARA DETECTAR EL OBJETO, SI ES NECESARIO
        ...
        code = self.arm.set_linear_motor_speed(200)
        #print('set linear motor speed, code={}'.format(code))
        code = self.arm.set_linear_motor_pos(50, wait=True)
    ...
    def _check(self):
        ...
        if abs(self.CURR_POS[0] - self.GOAL_POS[0]) < 2 and abs(self.CURR_POS[1] - self.GOAL_POS[1]) < 2: # 2mm
        ...
        if z < self.gripper_z_mm or z - 1 < self.GOAL_POS[2]:
            ...
            # Posicion de motor lineal para soltar el objeto
            code = self.arm.set_linear_motor_enable(True)
            #print('set linear motor enable, code={}'.format(code))
            # set speed 200mm/s
            code = self.arm.set_linear_motor_speed(200)
            #print('set linear motor speed, code={}'.format(code))
            # set position 700mm
            code = self.arm.set_linear_motor_pos(700, wait=True)
            #print('[wait]set linear motor pos, code={}'.format(code))

            self.arm.set_position(x=self.release_xyz[0], y=self.release_xyz[1], z=self.release_xyz[2], roll=180, pitch=0, yaw=0, speed=200, wait=True)
            time.sleep(4.5) 
            self.arm.set_position(roll=71.5, pitch=-90, yaw=-161.8, speed=100, wait=True)

            time.sleep(4)
            self.place()

            self.arm.set_position(roll=180, pitch=0, yaw=0, speed=100, wait=True)
            time.sleep(4)
            self.arm.set_position(x=self.detect_xyz[0], y=self.detect_xyz[1], z=self.detect_xyz[2], speed=100, wait=True)
            
            # Posicion de motor lineal para detectar el objeto
            code = self.arm.set_linear_motor_enable(True)
            #print('set linear motor enable, code={}'.format(code))
            # set speed 200mm/s
            code = self.arm.set_linear_motor_speed(200)
    ...
    def grasp(self, data):
        ...
        if self.GRASP_STATUS == 0:
            ...
            z = max(self.CURR_POS[2], 380)
            self.GOAL_POS = GOAL_POS
            self.arm.set_position(x=GOAL_POS[0], y=GOAL_POS[1], z=z, roll=GOAL_POS[3], pitch=GOAL_POS[4], yaw=GOAL_POS[5], speed=80, acc=1000, wait=False)
            self.GRASP_STATUS = 2
